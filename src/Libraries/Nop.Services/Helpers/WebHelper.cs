...
public partial class WebHelper : IWebHelper
{
    ...
    public virtual string ModifyQueryString(string url, string key, params string[] values)
    {
        ...
        if (isLocalUrl)
        {
            ...
            if (!string.IsNullOrEmpty(pathBase) && url.StartsWith(pathBase))
            {
                uriStr = $"{GetStoreLocation().TrimEnd('/')}{url.Substring(pathBase.ToString().Length)}";
            }
            else
            {
                uriStr = $"{GetStoreLocation().TrimEnd('/')}{url}";
            }
        }
