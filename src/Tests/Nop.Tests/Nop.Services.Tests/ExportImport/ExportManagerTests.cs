...
public class ExportManagerTests : ServiceTest
{
    ...
    public async Task CanExportOrdersXlsx()
    {
        ...
        // Row 2 in the worksheet represents the first exported order; bind assertions to that exact order.
        var exportedOrderId = manager.GetDefaultProperties.First(p => p.PropertyName == "OrderId").IntValue;
        var order = orders.FirstOrDefault(o => o.Id == exportedOrderId) ?? orders.First();
        ...
        if (order.ShippingAddressId is > 0 && !order.PickupInStore)
        {
            var testShippingAddress = await _addressService.GetAddressByIdAsync(order.ShippingAddressId.Value);
