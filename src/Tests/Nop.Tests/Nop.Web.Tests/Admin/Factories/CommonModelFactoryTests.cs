...
public class CommonModelFactoryTests : BaseNopTest
{
    ...
    public async Task TestGetNopLatestVersion()
    {
        ...
        // Result may be null when GitHub is unreachable in a test environment; we only
        // assert that no exception is thrown and the value, if present, is non-empty.
        if (nopLatestVersion != null)
            nopLatestVersion.Should().NotBeEmpty();
    }
