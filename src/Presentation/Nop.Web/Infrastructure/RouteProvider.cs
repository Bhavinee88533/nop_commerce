...
public partial class RouteProvider : BaseRouteProvider, IRouteProvider
{
    ...
    public virtual void RegisterRoutes(IEndpointRouteBuilder endpointRouteBuilder)
    {
        ...
        //session expire – signs out and redirects to login with sessionExpired flag
        endpointRouteBuilder.MapControllerRoute(name: "SessionExpire",
            pattern: $"{lang}/session/expire/",
            defaults: new { controller = "Customer", action = "SessionExpire" });
        ...
        //update (or remove) cart item quantity for a specific product. used on catalog pages. (AJAX)
        endpointRouteBuilder.MapControllerRoute(name: NopRouteNames.Ajax.UPDATE_CART_ITEM_QUANTITY,
            pattern: $"cart/updateitemqty/{{productId:min(0)}}/{{quantity:min(0)}}",
            defaults: new { controller = "ShoppingCart", action = "UpdateCartItemQuantity" });
