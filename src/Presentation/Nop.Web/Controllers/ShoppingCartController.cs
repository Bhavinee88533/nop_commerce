...
public partial class ShoppingCartController : BasePublicController
{
    ...
    //update (or remove) a cart item to an absolute quantity for a specific product. used on catalog pages. (AJAX)
    [HttpPost]
    public virtual async Task<IActionResult> UpdateCartItemQuantity(int productId, int quantity)
    {
        var customer = await _workContext.GetCurrentCustomerAsync();
        var store = await _storeContext.GetCurrentStoreAsync();
        var cart = await _shoppingCartService.GetShoppingCartAsync(customer, ShoppingCartType.ShoppingCart, store.Id);
        ...
        var cartItem = await _shoppingCartService.FindShoppingCartItemInTheCartAsync(cart, ShoppingCartType.ShoppingCart, product);
        if (cartItem == null)
            return Json(new { success = false, message = "Item not found in cart" });

        // Use product's OrderMaximumQuantity as the cap (backend service will also validate stock)
        if (product.OrderMaximumQuantity > 0)
            quantity = Math.Min(quantity, product.OrderMaximumQuantity);

        const int maxCartQty = 5;
        quantity = Math.Min(quantity, maxCartQty);
    ...
    public virtual async Task<IActionResult> UpdateCart(IFormCollection form)
    {
        ...
        var itemsWithNewQuantity = cart.Select(item => new
        {
            ...
            NewQuantity = itemIdsToRemove.Contains(item.Id)
                ? 0
                : int.TryParse(form[$"itemquantity{item.Id}"], out var quantity)
                    ? Math.Min(quantity, maxCartQty)
                    : item.Quantity,
            Item = item,
    ...
    [HttpPost, ActionName("Cart")]
    [FormValueRequired(FormValueRequirement.StartsWith, "removeitem-")]
    public virtual async Task<IActionResult> RemoveCartItem(IFormCollection form)
    {
        var model = new ShoppingCartModel();

        //get cart item identifier
        var cartItemId = 0;
        foreach (var formValue in form.Keys)
        {
            if (formValue.StartsWith("removeitem-", StringComparison.InvariantCultureIgnoreCase))
            {
                cartItemId = Convert.ToInt32(formValue["removeitem-".Length..]);
                break;
            }
        }
        ...
        var cartItem = cart.FirstOrDefault(item => item.Id == cartItemId);
        if (cartItem != null)
            await _shoppingCartService.DeleteShoppingCartItemAsync(cartItem);
    ...
    [HttpGet]
    public virtual async Task<IActionResult> RemoveCartItemDirect(int itemId)
    {
        if (!await _permissionService.AuthorizeAsync(StandardPermission.PublicStore.ENABLE_SHOPPING_CART))
            return RedirectToRoute(NopRouteNames.General.HOMEPAGE);
