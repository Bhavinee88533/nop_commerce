...
public partial class SettingModelFactory : ISettingModelFactory
{
    ...
    /// <summary>
    /// Prepare login settings model
    /// </summary>
    /// <returns>
    /// A task that represents the asynchronous operation
    /// The task result contains the login settings model
    /// </returns>
    protected virtual async Task<LoginSettingsModel> PrepareLoginSettingsModelAsync()
    {
        //load settings
        ...
        {
            DefaultLoginMethod = loginSettings.DefaultLoginMethod,
            AllowUserToChangeLoginMethod = loginSettings.AllowUserToChangeLoginMethod,
            RememberLastUsedMethod = loginSettings.RememberLastUsedMethod
        };

        //prepare available login methods (only OTP options)
        model.AvailableLoginMethods.Add(new SelectListItem
        {
            Text = "Email OTP (One-Time Password via Email)",
            Value = "OtpEmail"
        });
        model.AvailableLoginMethods.Add(new SelectListItem
        {
            Text = "Mobile OTP (One-Time Password via SMS)",
            Value = "OtpMobile"
        });
    ...
    public virtual async Task<CustomerUserSettingsModel> PrepareCustomerUserSettingsModelAsync(CustomerUserSettingsModel model = null)
    {
        ...
        //prepare login settings model
        model.LoginSettings = await PrepareLoginSettingsModelAsync();
