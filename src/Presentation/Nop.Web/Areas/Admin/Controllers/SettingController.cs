...
public partial class SettingController : BaseAdminController
{
    ...
    public virtual async Task<IActionResult> CustomerUser(CustomerUserSettingsModel model)
    {
        if (ModelState.IsValid)
        {
            ...
            loginSettings.DefaultLoginMethod = model.LoginSettings.DefaultLoginMethod;
            loginSettings.AllowUserToChangeLoginMethod = model.LoginSettings.AllowUserToChangeLoginMethod;
            loginSettings.RememberLastUsedMethod = model.LoginSettings.RememberLastUsedMethod;
            await _settingService.SaveSettingAsync(loginSettings);
