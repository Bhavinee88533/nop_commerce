...
public partial class ProductModelFactory : IProductModelFactory
{
    ...
    protected virtual async Task<ProductDetailsModel.AddToCartModel> PrepareProductAddToCartModelAsync(Product product, ShoppingCartItem updatecartitem)
    {
        ...
        var currentCustomer = await _workContext.GetCurrentCustomerAsync();
        var currentStore = await _storeContext.GetCurrentStoreAsync();
        var cartItems = await _shoppingCartService.GetShoppingCartAsync(currentCustomer, Nop.Core.Domain.Orders.ShoppingCartType.ShoppingCart, currentStore.Id);
        var existingCartItem = cartItems.FirstOrDefault(sci => sci.ProductId == product.Id);
        model.CartQuantity = existingCartItem?.Quantity ?? 0;

        //check if product is already in wishlist
        var wishlistItems = await _shoppingCartService.GetShoppingCartAsync(currentCustomer, Nop.Core.Domain.Orders.ShoppingCartType.Wishlist, currentStore.Id);
        model.IsInWishlist = wishlistItems.Any(sci => sci.ProductId == product.Id);
        ...
        bool forceRedirectionAfterAddingToCart = false)
    {
        ...
        var overviewCustomer = await _workContext.GetCurrentCustomerAsync();
        var overviewStore = await _storeContext.GetCurrentStoreAsync();
        var customerCartItems = await _shoppingCartService.GetShoppingCartAsync(overviewCustomer, Nop.Core.Domain.Orders.ShoppingCartType.ShoppingCart, overviewStore.Id);
        var customerWishlistItems = await _shoppingCartService.GetShoppingCartAsync(overviewCustomer, Nop.Core.Domain.Orders.ShoppingCartType.Wishlist, overviewStore.Id);
        var cartQuantities = customerCartItems
            .GroupBy(item => item.ProductId)
            .ToDictionary(group => group.Key, group => group.Sum(item => item.Quantity));
        var wishlistedProductIds = customerWishlistItems.Select(w => w.ProductId).ToHashSet();
        foreach (var product in products)
        {
            ...
            //custom wishlist items
            model.ProductToWishlist = await PrepareProductToWishlistModelAsync(product);
            model.IsInWishlist = wishlistedProductIds.Contains(product.Id);
            model.CartQuantity = cartQuantities.GetValueOrDefault(product.Id);
