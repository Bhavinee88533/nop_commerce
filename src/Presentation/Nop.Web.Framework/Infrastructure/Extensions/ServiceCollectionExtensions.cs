...
public static class ServiceCollectionExtensions
{
    ...
    public static void AddNopAuthentication(this IServiceCollection services)
    {
        ...
        authenticationBuilder.AddCookie(NopAuthenticationDefaults.AuthenticationScheme, options =>
        {
            ...
            // Session expires after 1 hour of inactivity; each request resets the timer
            options.SlidingExpiration = true;
            options.ExpireTimeSpan = TimeSpan.FromHours(1);
            // For AJAX/API requests: return JSON 401 instead of HTML redirect
            options.Events.OnRedirectToLogin = ctx =>
            {
                var isApiRequest = ctx.Request.Headers["Accept"].ToString().Contains("application/json")
                    ...
                    || ctx.Request.Path.StartsWithSegments("/otp");
                if (isApiRequest)
                {
                    ctx.Response.StatusCode = StatusCodes.Status401Unauthorized;
                    ctx.Response.ContentType = "application/json";
                    return ctx.Response.WriteAsync(
                        "{\"ok\":false,\"sessionExpired\":true,\"error\":\"Your session has expired. Please log in again.\",\"redirectUrl\":\"/otp/login\"}");
                }
                // Append sessionExpired=true so the login page can show the expiry popup
                var redirectUri = ctx.RedirectUri;
                if (!redirectUri.Contains("sessionExpired"))
                    redirectUri = redirectUri + (redirectUri.Contains('?') ? "&" : "?") + "sessionExpired=true";
                ctx.Response.Redirect(redirectUri);
                return Task.CompletedTask;
            };
            // For AJAX/API requests: return JSON 403 instead of HTML redirect to /page-not-found
            options.Events.OnRedirectToAccessDenied = ctx =>
            {
                ...
                if (isApiRequest)
                {
                    ...
                    ctx.Response.ContentType = "application/json";
                    return ctx.Response.WriteAsync(
                        "{\"ok\":false,\"error\":\"You do not have permission to access this resource.\"}");
                }
